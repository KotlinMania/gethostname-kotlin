# Claude Code Project Instructions — gethostname-kotlin

## Project Overview

This is **gethostname-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`gethostname`](https://crates.io/crates/gethostname).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/gethostname/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/gethostname>
...
Reference example: `serde-kotlin/tmp/serde/serde_core/src/private/mod.rs`
re-exports `Result` from `crate::lib::result`. The Kotlin tracking file lives at
[`serde-kotlin/src/commonMain/kotlin/io/github/kotlinmania/serde/core/private/Mod.kt`](https://github.com/KotlinMania/serde-kotlin/blob/main/src/commonMain/kotlin/io/github/kotlinmania/serde/core/private/Mod.kt).
