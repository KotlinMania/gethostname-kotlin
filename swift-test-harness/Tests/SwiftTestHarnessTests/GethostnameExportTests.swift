...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Gethostname.swiftmodule/`
...
//      `import Gethostname` above would fail at compile time.
//
//   2. The static archive `libGethostname.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Gethostname" }` and
//      `flattenPackage = "io.github.kotlinmania.gethostname"` configuration in
...
//      via the `GethostnameLibrary` product.
//
...
final class GethostnameExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Gethostname swift module imported cleanly")
    }
}
